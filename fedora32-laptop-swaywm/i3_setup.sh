...
#terminal emulators
TERM_CHOICES="xterm rxvt-unicode cancel"
PS3="Choose your terminal-emulator or choose cancel(number):"
select SELECTED_TERM in $TERM_CHOICES
do
  if [ $SELECTED_TERM = "cancel" ]; then
    break
  else
    PACKAGES="$PACKAGES $SELECTED_TERM"
    echo "You have chosen $SELECTED_TERM"
    break
  fi
done
echo
...
#i3 related packages
echo "Do you want to install [i3 rofi picom feh polybar network-manager-applet arandr pavucontrol dunst ranger xbacklight]?"
read -p "(y/n)" yn
if [[ $yn = [yY] ]]; then
  PACKAGES="$PACKAGES i3 rofi picom feh polybar network-manager-applet arandr pavucontrol dunst ranger xbacklight"
fi
echo
